...
格式要求（重要：海选号和分数都是数字，必须用裁判名分隔）：
1. 如果只有海选号，格式：海选号
   例如：001
   或者：1

2. 如果有海选号和分数，格式：海选号 裁判名 分数
   ⚠️ 注意：海选号在前，裁判名在中间，分数在后（分数通常是小数如8.8）
   例如：001 Judge A 8.8
   或者：1 张老师 9.5
   或者：20 李老师 7.5

3. 如果某个海选号的所有老师都是同一分数，格式：海选号 所有老师 分数
   例如：001 所有老师 8.5

4. 每行一个条目，用换行分隔
5. 只输出数据，不要添加其他说明文字
6. 不需要选手名称，只需要海选号
7. 如果表格有多列，通常第一列是海选号，中间列是裁判名，最后一列是分数
当前打分是由 xx 打分
