  ...
  return (
    <div className="container">
      <div style={{ marginBottom: '20px' }}>
        ...
        <div className="card">
          <h1 style={{ margin: 0, color: 'var(--dark)' }}>{matchData.match.name}</h1>
          <div style={{ marginTop: '8px', color: '#666', fontSize: '14px' }}>
            创建于：{new Date(matchData.match.createdAt).toLocaleString('zh-CN')}
            {' | '}
            更新于：{new Date(matchData.match.updatedAt).toLocaleString('zh-CN')}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', marginTop: '20px' }}>
        <button className="btn btn-outline" onClick={handleUndo} disabled={historyIndex < 0}>
          ↶ Undo
        </button>
        <button
          className="btn btn-outline"
      ...
      <button className="fab" onClick={() => setShowAiPrompt(true)} title="AI 助手">
        🤖
      </button>
    </div>
  );
